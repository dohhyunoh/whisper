...
import { setTrialEndsAt, TRIAL_DURATION_MS } from '@/utils/trial';
...
export default function FreemiumUpgradeScreen() {
  ...
  const handleContinue = () => {
    ...
    // Gift existing freemium users 30 days of full access.
    const endsAt = Date.now() + TRIAL_DURATION_MS;
    setTrialEndsAt(endsAt);
    dispatch({ type: 'GRANT_TRIAL', payload: endsAt });
    markV2MigrationSeen();
    ...
    >
      ...
      >
        <View style={styles.top}>
          ...
          >
            <Ionicons name="gift" size={14 * s} color="#3A6B80" />
            <Text style={[styles.eyebrowText, { fontSize: 12 * s }]}>A gift for you</Text>
          </Animated.View>
        ...
        <View style={styles.middle}>
          ...
          >
            <Text style={[styles.body, { fontSize: 16 * s, lineHeight: 24 * s }]}>
              Whisper is now a curated, premium experience — a scarce, intentional batch of{' '}
              <Text style={styles.bodyEmphasis}>10 quotes daily</Text>, tuned to your emotional state.
              For being with us early, the next{' '}
              <Text style={styles.bodyEmphasis}>30 days are on us</Text>.
            </Text>
          ...
          >
            <Text style={[styles.trialText, { fontSize: 14 * s }]}>
              Full access, free for 30 days. Nothing to do — enjoy.
            </Text>
          </Animated.View>
        </View>
