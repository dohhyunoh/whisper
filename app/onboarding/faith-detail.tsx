...
export default function FaithDetailScreen() {
  ...
  const lightSource = state.user?.lightSource || 'Up';
  const options = optionsMap[lightSource] || optionsMap.Up;

  useEffect(() => {
    posthog.capture(Events.ONBOARDING_SCREEN_VIEWED, { screen_name: 'faith_detail' });
  }, []);

  return (
    <OnboardingLayout
      title={"Tell us a little\nmore..."}
      subtitle="So we can find the right voices for you."
      onContinue={() => {
        if (selected) {
          dispatch({
            type: 'SET_USER',
