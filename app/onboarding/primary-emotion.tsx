...
const options = ['Anxious', 'Sad', 'Angry', 'Numb', 'Exhausted', 'Hopeful'];
...
export default function PrimaryEmotionScreen() {
  ...
  useEffect(() => {
    posthog.capture(Events.ONBOARDING_SCREEN_VIEWED, { screen_name: 'primary_emotion' });
  }, []);

  return (
    <OnboardingLayout
      title={"If your heart could\nspeak one word..."}
      subtitle="No judgment. Just honesty."
      onContinue={() => {
        if (selected) {
          dispatch({
            type: 'SET_USER',
            payload: { ...defaultUserData, ...state.user, primaryEmotion: selected },
          });
          router.push('/onboarding/emotion-root');
        }
      }}
          ...
          return (
            <Pressable
              ...
              onPress={() => {
                ...
                setSelected(option);
                posthog.capture(Events.ONBOARDING_CHOICE_MADE, { screen: 'primary_emotion', choice: option });
              }}
            >
