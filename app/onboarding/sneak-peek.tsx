...
import { pickFirstQuote, setFirstQuote } from '@/utils/first-quote';
...
export default function SneakPeekScreen() {
  ...
  // The user's very first quote — shared picker (also the paywall's relaunch
  // fallback), scoring on interest tags, current emotion, and tone preference.
  const quote = useMemo(
    () =>
      pickFirstQuote(allQuotes, {
        interests,
        mood: state.user?.primaryEmotion,
        tone: state.user?.tonePreference,
      })!,
